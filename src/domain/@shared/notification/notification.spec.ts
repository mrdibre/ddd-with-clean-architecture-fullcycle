...
describe("Unit test for Notification", () => {
  test("Should create errors", () => {
    ...
    expect(notification.messages("customer")).toBe("customer: error message, error message2");

    const error3 = { message: "error message3", context: "order" };
    notification.addError(error3);

    expect(notification.messages("customer")).toBe("customer: error message, error message2");

    expect(notification.messages()).toBe("customer: error message, error message2, order: error message3");
  });
